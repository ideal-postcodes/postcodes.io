...
```json
{
  ...
  "result": {
    ...
    "index_of_multiple_deprivation": 22386,
    "admin_ward": "Hextable",
    "ced": "Swanley",
...
Postcodes.io serves three upstream datasets:

- **ONS Postcode Directory (ONSPD)** — UK postcodes with administrative, electoral, statistical and health geographies. Updated quarterly (February, May, August, November). Currently loaded: **August 2026**.
- **OS Open Names** — places, settlements and other gazetteer entries for Great Britain. Updated quarterly. Currently loaded: **July 2026**.
- **Scottish Postcode Directory (SPD)** — postcodes with Scottish-specific geographies (council areas, Scottish Parliamentary constituencies, Data Zones, SIMD, etc.). Updated twice a year. Currently loaded: **2026/2**.

These versions describe the public API and the latest Docker images; a full release history is in the [CHANGELOG](https://github.com/ideal-postcodes/postcodes.io/blob/master/CHANGELOG.md) on GitHub.

:::note

Beyond the dates listed in the changelog, please be aware that the real-world dataset may change. Existing postcodes and places may be amended or discontinued. This information will not be captured until the next available update.
:::
